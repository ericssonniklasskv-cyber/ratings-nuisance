...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  if (!item) return <main className="app-shell"><AppHeader admin={profile?.is_admin ?? false} /><div className="empty-state"><h1>Titeln kunde inte hämtas</h1><p>Kontrollera TMDb-nyckeln eller försök igen senare.</p><Link href="/rate">Tillbaka till sökningen →</Link></div></main>;

  const { data: stored } = await supabase.from("titles").select("id").eq("media_type", type).eq("tmdb_id", id).maybeSingle();
  const [{ data: ratings }, { data: references }, { data: officialRating }] = await Promise.all([
  ...
  return <main className="app-shell">
    ...
    <div className="title-hero">
      <div className="title-hero-poster"><Poster path={item.posterPath} title={item.title} priority /></div>
      <div><p className="kicker">{type === "movie" ? "FILM" : "TV-SERIE"} · {item.releaseYear ?? "ÅR OKÄNT"} · TMDb #{id}</p><h1>{item.title}</h1></div>
    </div>
    ...
    <section className="group-ratings"><h2>Officiellt gruppbetyg</h2>{officialRating ? <p className="official-rating"><strong>{Number(officialRating.score).toFixed(1)}</strong><span>· {officialRating.rating_count} betrodda betyg</span></p> : <p className="muted">Inget officiellt gruppbetyg ännu.</p>}<h3>Alla betyg</h3>{ratings?.length ? <div className="group-list">{ratings.map((rating) => <div className="group-row" key={rating.id}><span>{rating.profiles?.display_name || "Medlem"}{rating.user_id === user!.id ? " (du)" : ""}</span><strong>{Number(rating.score).toFixed(1)}</strong></div>)}</div> : <p className="muted">Ingen har betygsatt titeln ännu.</p>}</section>
  </main>;
}

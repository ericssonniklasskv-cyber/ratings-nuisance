...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  const [{ data: ratings }, { data: reference }, { data: officialRating }] = await Promise.all([
  ...
  return <main className="app-shell">
    ...
    <div className="detail-layout">
      ...
      <div className="detail-main">
        ...
        <section className="group-ratings">
          <h2>Officiellt gruppbetyg</h2>
          {officialRating ? <p className="official-rating"><strong>{Number(officialRating.score).toFixed(1)}</strong><span>· {officialRating.rating_count} betrodda betyg</span></p> : <p className="muted">Inget officiellt gruppbetyg ännu.</p>}
          <h3>Alla betyg</h3>
          {ratings?.length ? <div className="group-list">{ratings.map((rating) => <div className="group-row" key={rating.id}><span>{rating.profiles?.display_name || "Medlem"}{rating.user_id === user!.id ? " (du)" : ""}</span><strong>{Number(rating.score).toFixed(1)}</strong></div>)}</div> : <p className="muted">Ingen har betygsatt titeln ännu.</p>}
        </section>
      </div>
    </div>
  </main>;

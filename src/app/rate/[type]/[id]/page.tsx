...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <main className="app-shell">
    ...
    <div className="title-hero">
      <div className="title-hero-poster"><Poster path={item.posterPath} title={item.title} size="detail" priority /></div>
      <div><p className="kicker">{type === "movie" ? "MOVIE" : "TV"} · {item.releaseYear ?? "YEAR UNKNOWN"}</p><h1>{item.title}</h1></div>
    </div>
